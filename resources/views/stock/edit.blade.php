<x-app-layout>
    <x-slot name="title">Heladería - Edita Mov.Stock</x-slot>
        ...
        <div class="mx-4 md:mx-8 w-full bg-gray-100 overflow-y-auto shadow-lg rounded-lg opacity-85">
            ...
            <div class="grid overflow-hidden grid-cols-2 grid-rows-1 gap-1 w-full">
                ...
                <div>
                    ...
                    <div class="mx-5 mt-1">
                        ...
                        <x-select id="selTipoMovim" class="w-full h-9" disabled>
                            <x-slot name="options">
                ...
                <div class="shadow-md mr-5 my-2 bg-gray-50 overflow-y-auto border-b border-gray-200 sm:rounded-lg">
            ...
            <div class="flex justify-between mt-2 mb-3">
                ...
                <div class="flex justify-end mr-6">
                    ...
                    <x-link-cancel href="{{ $rutaInfoMovs }}" class="mr-4" id="btnCancela">
                        Cancela
                    </x-link-cancel>
                    <x-link-salir href="{{  route('stock.infomovimientos') }}">
                        Volver
                    </x-link-salir>
                </div>
            </div>
    ...
    @push('scripts')
        <script>
            var _STOCK = {
                _esEdit: true,
                comprobante_id: parseInt("{{ $comprobante->id }}"),
                usuario_id: parseInt("{{ Auth::user()->id }}"),
                sucursal_id: parseInt("{{ $comprobante->sucursal_id }}"),
                turno_id: parseInt("{{ $comprobante->turno_id }}"),
                turno_sucursal: parseInt("{{ $comprobante->turno_sucursal }}"),
                pathUpdateComprob: "{{ route('comprobante_stock.update', 0) }}",
                pathUpdateDetalle: "{{ route('detalle_comprobante_stock.update', 0) }}",
