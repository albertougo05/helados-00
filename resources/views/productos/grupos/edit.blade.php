<x-app-layout>
    ...
    <div class="flex mt-8 mb-6">
        <div class="mx-auto w-full md:w-5/6 bg-gray-100 overflow-hidden shadow-lg sm:rounded-lg opacity-83">
            ...
            <form method="POST" 
                  action="{{ route('producto_grupo.update', $grupo->id) }}"
                  autocomplete="off">
                @csrf
                @method('PUT')
                ...
                <div class="grid overflow-auto grid-cols-6 gap-3 px-6 pb-3">
                    ...
                    <div>   <!-- Row 2 Col 4 -->
                        ...
                        <x-select id="estado" name="estado" class="w-full h-9 mt-0">
                            <x-slot name="options">
                                <option value="1" {{ $grupo->estado == 1 ? 'selected' : '' }}>
                                    Activo
                                </option>
                                <option value="0" {{ $grupo->estado == 0 ? 'selected' : '' }}>
                                    Baja
                                </option>
                            </x-slot>
                        </x-select>
