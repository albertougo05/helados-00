...
var setInputBuscar = function setInputBuscar(filtro) {
  var text = filtro[2].replace('%', '');
  document.querySelector('input#buscar').value = text;
};

var setSelect = function setSelect(filtro, listado) {
  var valor = filtro[2];
  var select = document.querySelector('select#' + filtro[0]); // Buscar en listado

  var idx = listado.findIndex(function (e) {
    return e.id == valor;
...
window.onload = function () {
  if (__sessionStatusMsj) {
    (0,_utils_MensajeAlerta__WEBPACK_IMPORTED_MODULE_0__["default"])(__sessionStatusMsj, 'success');
  }

  var btnFiltrar = document.querySelector('button#btnFiltrar');
  btnFiltrar.addEventListener('click', function (e) {
    var prov = document.querySelector('select#proveedor_id').value;
    var tipo = document.querySelector('select#tipo_producto_id').value;
    var grup = document.querySelector('select#grupo_id').value;
    var busq = document.querySelector('input#buscar').value || '';
    var url = '/producto/filtrado?prov=' + prov + '&tipo=' + tipo;
    url += '&grupo=' + grup + '&buscar=' + busq;
    location.assign(url);
  });
  var btnBuscar = document.querySelector('button#btnBuscar');
  ...
  if (__filtros) {
    var btnQuitar = document.querySelector('button#btnQuitarBusq');
    btnQuitar.addEventListener('click', function () {
      location.assign('/producto');
    }); // setear input y selec

    var _iterator = _createForOfIteratorHelper(__filtros),
        _step;
